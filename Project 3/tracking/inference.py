...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in allGhostPositions:
            newdist = self.getPositionDistribution(gameState, oldPos)
            for newPos, likelihood in newdist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            if particle in tmp:
                ...
                tmp[particle] = observProb
        tmp.normalize()
        if tmp.total() > 0:
            self.particles = [tmp.sample() for _ in range(self.numParticles)]
        else: 
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        newParts = []
        for particle in self.particles:
            final = self.getPositionDistribution(gameState, particle)
            sample = final.sample()
