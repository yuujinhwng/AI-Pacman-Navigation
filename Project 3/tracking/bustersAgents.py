...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            max.append(belief.argMax())
        goalCoordinate, goalProbability = None, 0
        for i, coordinate in enumerate(max):
            if livingGhostPositionDistributions[i][coordinate] >= goalProbability:
                goalCoordinate, goalProbability = coordinate, livingGhostPositionDistributions[i][coordinate]

        temp = []
        for action in legal:
            next_loc = Actions.getSuccessor(pacmanPosition, action)
            temp.append((self.distancer.getDistance(next_loc, goalCoordinate), action))
        return min(temp)[1]
