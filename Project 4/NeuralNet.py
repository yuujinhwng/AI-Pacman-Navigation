...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """YOUR CODE"""
        inActs = [1.0] + inActs
        val = self.getWeightedSum(inActs)
        inActs = inActs[1:]
        output = self.sigmoid(val)
        return output
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """YOUR CODE"""
        inActs = [1.0] + inActs
        for i, (weight, old) in enumerate(zip(self.weights, self.weights)):
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            last = [node.sigmoidActivation(last) for node in layer]
            output.append(last)
        return output
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            """FILL IN - neural net output list computation"""
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    error = 0.0
                    error = sum(nextLayer[n].weights[neuronNum + 1] * deltas[0][n] for n in range(len(nextLayer)))
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for input, output in examplesTest:
        ...
        equal = all(round(n) == o for n, o in zip(nnetOut, output))
        if equal:
            testCorrect += 1
        else:
            testError += 1
